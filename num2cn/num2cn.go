...
func CN2Int64(cn string) (int64, error) {
	chars := []rune(cn)
	return cnToInt64(chars)
}

func cnToInt64(chars []rune) (res int64, err error) {
		...
		for idx := len(chars) - 1; idx >= 0; idx-- {
			if chars[idx] == specialUnit.cn {
				...
				if idx == len(chars)-1 {
					...
					right, err = cnToInt64(chars[idx+1:])
					if err != nil {
						return 0, err
					}
				}
...
func CN2Float64(cn string) (float64, error) {
	...
	res, err := cnToFloat64(chars)
	return res * sign, err
}

func cnToFloat64(chars []rune) (res float64, err error) {
	// deal with decimal point
	for idx := len(chars) - 1; idx >= 0; idx-- {
		if chars[idx] == decimalPoint {
			integralPart, err := cnToFloat64(chars[0:idx])
			if err != nil {
				return 0, err
			}
	...
	for _, specialUnit := range cnExtremeUnits {
		...
		for idx := len(chars) - 1; idx >= 0; idx-- {
			if chars[idx] == specialUnit.cn {
				left, err = cnToFloat64(chars[:idx])
				if err != nil {
					return 0, err
				}
				if idx == len(chars)-1 {
					...
					right, err = cnToFloat64(chars[idx+1:])
					if err != nil {
						return 0, err
					}
				}
...
// Float64ToCN : convert float64 into Chinese number
// 由于float64固然存在的精度问题，本函数可能不会特别精准。所以小数部分最多精确到6位。
func Float64ToCN(num float64) string {
	if num < 0 {
		return string(cnNegativePrefix) + Float64ToCN(-num)
	}
