...
import (
	"fmt"
	"github.com/bySouffle/bysutil/log"
...
func NewClient(opts ...Options) *Client {
	client := &Client{
		SendMsg:    make(chan []byte, SendQueueLen),
		ReceiveMsg: make(chan []byte, ReceiveQueueLen),
...
func (c *Client) Write() {
	...
	for {
		if len(c.SendMsg) == SendQueueLen {
			log.Warn("[WebSocket] SendQueue full")
		}
		select {
